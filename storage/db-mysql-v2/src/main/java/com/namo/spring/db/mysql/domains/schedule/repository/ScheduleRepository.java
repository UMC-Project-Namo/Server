...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Query("SELECT NEW com.namo.spring.db.mysql.domains.schedule.dto.MeetingScheduleQueryDto(s.id, s.title, p, COUNT(p)) FROM Schedule s JOIN s.participantList p WHERE p.member = :member AND s.scheduleType = 1 ORDER BY s.period.startDate")
